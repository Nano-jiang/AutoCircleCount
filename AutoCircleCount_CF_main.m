...
% %% 可调参数：
% 筛选窗口秒数 2-30 s 在 112-113 行可以修改
% 考虑不同小鼠完成一圈的时间可能存在较大差异
% 一般完成得快 2 s 足够，完成得慢需要 30 s
% 所以这一步是给完成圈数一个时间阈值

%% 环境路径配置
current_root = fileparts(mfilename('fullpath'));

all_sub_folders = genpath(current_root);

addpath(all_sub_folders);

%% 小鼠轨迹
AutoFramesCount_CF_New
